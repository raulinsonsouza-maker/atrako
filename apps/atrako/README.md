...
3. **Variáveis de ambiente (opcionais)**
   ...
   - Autenticação local: usuários são criados em **Administração → Usuários** com username e senha.
     Para o primeiro administrador, use temporariamente `INTERNAL_AUTH_BOOTSTRAP=true`,
     `INTERNAL_BOOTSTRAP_USERNAME` e `INTERNAL_BOOTSTRAP_PASSWORD` (e, ao adotar
     um administrador legado, `INTERNAL_BOOTSTRAP_LEGACY_USER_ID`); remova todas
     as variáveis de bootstrap
