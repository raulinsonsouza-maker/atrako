...
export async function PUT(
  ...
  await writeAuditLog({
    ...
    metadata: {
      ...
      changedFields: [
        ...(body.tipo !== undefined ? ["tipo"] : []),
        ...(body.dominio !== undefined ? ["dominio"] : []),
        ...(body.ativo !== undefined ? ["ativo"] : []),
        ...Object.keys(body.credenciais ?? {}).map((field) => `credenciais.${field}`),
      ],
    },
  });
    ...
    try {
      ...
      if (typeof result === "object" && result !== null && "ok" in result && result.ok === true) {
        await writeAuditLog({
          ...
          actorInternalUserId: access.user.id,
          metadata: { clientId: id, operationType: "test" },
        });
      }
