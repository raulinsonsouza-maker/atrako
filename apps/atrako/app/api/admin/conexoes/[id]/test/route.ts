...
async function testMeta(accessToken: string): Promise<{ ok: boolean; detail: string }> {
  try {
    const url = `https://graph.facebook.com/v22.0/me?fields=id,name&access_token=${encodeURIComponent(accessToken)}`;
...
export async function POST(
  ...
  if (c.plataforma === "META") {
    ...
    if (result.ok) {
      await writeAuditLog({
        ...
        metadata: { targetResourceId: id, operationType: "test", platform: c.plataforma },
      });
    }
  ...
  if (c.plataforma === "LINKEDIN") {
    if (!c.linkedinAccessToken) {
      return NextResponse.json({ ok: false, detail: "OAuth LinkedIn não concluído. Use o botão Conectar LinkedIn." });
    }
    const { getValidLinkedinAccessToken, testLinkedinConnection } = await import(
      "@/lib/linkedin/linkedinClient"
    );
    try {
      const token = await getValidLinkedinAccessToken(c.id);
      const result = await testLinkedinConnection(token);
      if (!result.ok) return NextResponse.json({ ok: false, detail: result.error ?? "Falha na API LinkedIn" });
      await writeAuditLog({
        action: "INTEGRATION_CONNECTION_TESTED",
        actorInternalUserId: access.user.id,
        metadata: { targetResourceId: id, operationType: "test", platform: c.plataforma },
      ...
      return NextResponse.json({
        ok: true,
        detail: `Autenticado — ${result.accounts.length} conta(s) de anúncios visível(is)`,
      });
    } catch (e) {
      return NextResponse.json({ ok: false, detail: e instanceof Error ? e.message : "Falha de rede" });
    }
  }
