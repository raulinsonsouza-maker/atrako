...
async function getAuthorization(request: NextRequest) {
  ...
  if (process.env.IMPORT_LEADS_SECRET && secret === process.env.IMPORT_LEADS_SECRET) {
    return { authorized: true, internalUser: null };
  }
  const access = await requireInternalAdmin();
  return { authorized: Boolean(access.user && !access.response), internalUser: access.user };
}
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (contentType.includes("multipart/form-data")) {
      ...
      if (authorization.internalUser) {
        await writeAuditLog({
          action: "LEADS_IMPORTED",
          actorInternalUserId: authorization.internalUser.id,
          metadata: { clientId: clienteId, operationType: "csv_upload" },
        });
      }
    ...
    if (authorization.internalUser) {
      await writeAuditLog({
        action: "LEADS_IMPORTED",
        actorInternalUserId: authorization.internalUser.id,
        metadata: { clientId: clienteId, operationType: "sheets_import" },
      });
    }
