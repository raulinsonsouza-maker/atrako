...
/** Sync manual Shopify → CRM + pedidos + catálogo + financeiro (via ingest). */
export async function POST(request: NextRequest) {
  ...
  try {
    const result = await syncShopifyWorkspace(workspaceId, {
      daysBack: typeof body.daysBack === "number" ? body.daysBack : 90,
      maxPages: typeof body.maxPages === "number" ? body.maxPages : 10,
    });
