...
export async function GET(
  ...
  if (nivel === "campanhas") {
    ...
    const previousRows = comparePrevious ? await prisma.fatoMidiaDiario.findMany({
      where: { clienteId: id, data: { gte: previousStart, lte: previousEnd }, ...(canalFilter ? { canal: canalFilter } : {}), NOT: { campaignName: "" } },
    }) : [];
    const previousByCampaign = new Map<string, { nome: string; investimento: number; faturamento: number; purchases: number }>();
    for (const row of previousRows) {
      const nome = row.campaignName.trim() || "Campanha sem nome";
      const key = comparePrevious && row.campaignId ? row.campaignId : nome;
      const value = previousByCampaign.get(key) ?? { nome, investimento: 0, faturamento: 0, purchases: 0 };
      value.investimento += Number(row.investimento);
      value.faturamento += Number(row.websitePurchasesConversionValue);
      value.purchases += row.purchases;
      previousByCampaign.set(key, value);
    }
    ...
    for (const fato of fatos) {
      ...
      // Keep the historical campaign-name key for ordinary/drill-down calls.
      // The pilot comparison alone needs the stable platform identifier.
      const campaignKey = comparePrevious && fato.campaignId ? fato.campaignId : nome;
      const dateKey = fato.data.toISOString().slice(0, 10);
      const existing = byCampanha.get(campaignKey);
      if (existing) {
    ...
    const campanhas = Array.from(byCampanha.entries())
      .map(([campaignKey, v]) => {
        ...
        return {
          ...
          ...(comparePrevious ? (() => {
            const previous = previousByCampaign.get(campaignKey);
            return {
              comparison: {
                state: !previous ? "new" : "continuing",
                investimento: percentChange(v.investimento, previous?.investimento ?? 0),
                faturamento: percentChange(v.faturamento, previous?.faturamento ?? 0),
                purchases: percentChange(v.purchases, previous?.purchases ?? 0),
              },
            };
          })() : {}),
        };
    ...
    const stoppedCampaigns = comparePrevious
      ? Array.from(previousByCampaign.entries()).filter(([key]) => !byCampanha.has(key)).map(([, p]) => ({
        nome: p.nome, status: "PAUSADA", diasAtivos: 0, investimento: 0, impressoes: 0, cliques: 0, leads: 0,
        purchases: 0, faturamento: 0, conversas: 0, profileVisits: 0, resultType: "alcance", resultados: null,
        custoResultado: null, cpl: null, cpa: null, ticketMedio: null, roas: null, ctr: null,
        comparison: { state: "stopped", investimento: -100, faturamento: -100, purchases: -100 },
      }))
      : [];
