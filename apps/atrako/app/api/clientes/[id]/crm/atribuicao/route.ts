...
export async function GET(
  ...
  // Hierarquia Google UTM (campanha → grupo de anúncios via utmCampaign/utmContent do dadosMarketing)
  type GoogleAdgroupBucket = Bucket & { name: string };
  type GoogleCampaignBucket = Bucket & { name: string; adgroups: Map<string, GoogleAdgroupBucket> };
  const googleUtmHierMap = new Map<string, GoogleCampaignBucket>();
  ...
  for (const lead of leads) {
    ...
    // Fallback: se status é null, deriva do nome da etapa (CRMs que não setam status)
    const isWon = lead.status === "won" ||
      (lead.status === null && (
        etapaLower.includes("venda") ||
        (etapaLower.includes("contrato") && !etapaLower.includes("cancelado")) ||
        etapaLower.includes("assinado")
      ));
    const isLost = lead.status === "lost" ||
      (lead.status === null && (
        etapaLower.includes("perdido") ||
        etapaLower.includes("cancelado") ||
        etapaLower.includes("descartado") ||
        etapaLower.includes("sem interesse") ||
        etapaLower.includes("inativo")
      ));
    ...
    // Hierarquia Google UTM: campanha → grupo de anúncios via utmCampaign/utmContent
    if (canal === "GOOGLE") {
      const utmCamp = mkt?.utmCampaign?.trim() || null;
      if (utmCamp) {
        let gc = googleUtmHierMap.get(utmCamp);
        if (!gc) { gc = { name: utmCamp, ...emptyBucket(), adgroups: new Map() }; googleUtmHierMap.set(utmCamp, gc); }
        applyBucket(gc);
        const utmContent = mkt?.utmContent?.trim() || null;
        if (utmContent) {
          let ag = gc.adgroups.get(utmContent);
          if (!ag) { ag = { name: utmContent, ...emptyBucket() }; gc.adgroups.set(utmContent, ag); }
          applyBucket(ag);
        }
      }
    }
  ...
  // ── porGoogleUtmHier (campanha UTM → grupos de anúncios → leads) ────────────
  const porGoogleUtmHier = [...googleUtmHierMap.values()]
    .map((camp) => ({
      campaignName: camp.name,
      ...toRow(camp.name, camp),
      adgroups: [...camp.adgroups.values()]
        .map((ag) => ({
          adgroupName: ag.name,
          ...toRow(ag.name, ag),
        }))
