...
import { clientOwnsMetaAdset } from "@/lib/meta/adsetAccess";
...
export async function GET(
  ...
  if (!(await clientOwnsMetaAdset(clienteId, adsetId))) {
    return NextResponse.json({ error: "Conjunto não encontrado" }, { status: 404 });
  }
  ...
  if (!creds?.token) {
    return NextResponse.json({ error: "Dados Meta indisponíveis" }, { status: 503 });
  }
