...
export async function GET(
  ...
  const access = await requireClienteAccess(_request, id, "read");
...
export async function POST(
  ...
  if (access.internalUser) {
    await writeAuditLog({
      action: "PAUTA_CREATED",
      actorInternalUserId: access.internalUser.id,
      metadata: { clientId: id, operationType: "create", changedFields: ["titulo", "descricao", "semanaIso", "ano", "prioridade", "dataFim"] },
    });
  }
