...
async function fetchPainelEspecial(
  ...
  if (comparePrevious) params.set("compare", "previous");
  if (comparePrevious && preset) params.set("comparisonPreset", preset);
  let res = await fetch(`/api/clientes/${id}/painel-especial?${params.toString()}`);
  // O comparativo é opcional: token ausente/expirado nunca deve bloquear os
  // dados principais do cliente.
  if (res.status === 401 && comparePrevious) {
    params.delete("compare");
    params.delete("comparisonPreset");
    res = await fetch(`/api/clientes/${id}/painel-especial?${params.toString()}`);
  }
...
export function ClienteDashboard({ id, portalMode = false }: { id: string; portalMode?: boolean }) {
  ...
  const { data: internalSession, isFetched: internalSessionFetched } = useQuery<{ role: "ADMIN" | "ANALYST" } | null>({
    queryKey: ["internal-session"],
    queryFn: async () => {
      const response = await fetch("/api/internal/me");
      if (!response.ok) return null;
      const body = await response.json() as { role?: "ADMIN" | "ANALYST" | null };
      return body.role ? { role: body.role } : null;
    },
    enabled: !portalMode,
    staleTime: 5 * 60 * 1000,
    retry: false,
  });
  const internalSessionResolved = portalMode || internalSessionFetched;
  const canUseAnalyst = Boolean(internalSession?.role);

  const socialMediaOnlyRef = React.useRef(false);

  const triggerSync = React.useCallback(async (opts?: { background?: boolean }) => {
    const background = opts?.background ?? false;
    if (syncStatus === "syncing") return;
    setSyncStatus("syncing");
    const isSMOnly = socialMediaOnlyRef.current;
    try {
      if (isSMOnly) {
        // Social Media Only: nenhuma conta de tráfego pago — invalida o cache do
        // painel de Instagram para forçar nova leitura da API do IG.
        await queryClient.invalidateQueries({ queryKey: ["social-media", id] });
        setSyncStatus("done");
  ...
  React.useEffect(() => {
    if (portalMode || !canUseAnalyst) return;
  ...
  const socialMediaOnly = isSocialMediaOnly(cliente);
  const hotelPilotEnabled = !portalMode && cliente?.inPilotAvailable === true;
  React.useEffect(() => {
    if (!internalSessionResolved) return;
    const requested = new URLSearchParams(window.location.search).get("analyst") === "1";
    setAnalystOpen((current) =>
      hotelPilotEnabled && canUseAnalyst && requested
        ? true
        : hotelPilotEnabled && canUseAnalyst
          ? current
          : false
    );
  }, [hotelPilotEnabled, canUseAnalyst, internalSessionResolved]);
  const hasLinkedin = Boolean(
    (cliente as { contas?: { plataforma?: string; accountIdPlataforma?: string | null }[] } | undefined)?.contas?.some(
      (ct) => ct.plataforma === "LINKEDIN" && ct.accountIdPlataforma
    )
  );

  // Mantém a ref em sincronia para uso dentro de useCallback sem deps extras.
  React.useEffect(() => { socialMediaOnlyRef.current = socialMediaOnly; }, [socialMediaOnly]);
  ...
  const { data: metaAdsData, isLoading: metaAdsLoading, error: metaAdsError } = useQuery({
    ...
    enabled: !!id && canal === "meta" && internalSessionResolved,
  });
  ...
  const { data: painelEspecial } = useQuery({
    queryKey: ["painel-especial", id, canal, presetPeriodo, dateFilter.dataInicio, dateFilter.dataFim, chartAgrupamento, hotelPilotEnabled ? "pilot-comparison" : "standard", hotelPilotEnabled ? presetPeriodo : "none", portalMode ? "portal" : "internal"],
    queryFn: () => fetchPainelEspecial(id, canal as "geral" | "meta" | "google", dateFilter, presetPeriodo, isLongPeriod ? undefined : chartAgrupamento, hotelPilotEnabled),
...
function formatPercentage(value: number) {
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col gap-2" ref={filterRef}>
        ...
        <div className="flex items-center gap-2">
          ...
          {false && !portalMode && canal !== "geral" && canal !== "imoveis" && canal !== "crm" && (() => {
            ...
            const saldoNum: number | null = value ?? null;
            const isCritical = !isGastoMes && saldoNum !== null && (saldoNum as number) < 100;
      ...
      {!analystOpen && canal === "meta" && subView === "criativos" && (
      ...
      {/* ── Social Media (Instagram Orgânico — sub-tab do Meta, ou tela principal para Social Media Only) ── */}
      {!analystOpen && ((canal === "meta" && subView === "social-media") || socialMediaOnly) && id && (
        <SocialMediaPanel clienteId={id} dateFilter={dateFilter} />
      )}

      {/* ── Google Ads: Keywords (Criativos tab) ── */}
      {!analystOpen && canal === "google" && subView === "criativos" && (
        <GoogleKeywordsPanel
          data={
            googleKeywordsData ?? {
              keywords: [],
      ...
      {!analystOpen && canal === "imoveis" && isMiguelImoveis(cliente) && (
        <ImoveisPanel clienteId={id} dateFilter={dateFilter} />
      )}


      {/* ── CRM tab ── */}
      {!analystOpen && canal === "crm" && id && (
        <CrmTab
          clienteId={id}
          dateRange={{
      ...
      {/* ── E-commerce tab ── */}
      {!analystOpen && canal === "ecommerce" && id && (
        <EcommercePanel
          clienteId={id}
          dateRange={{
            from: dateFilter.dataInicio ?? new Date(new Date().getFullYear(), 0, 1).toISOString().slice(0, 10),
      ...
      {!analystOpen && canal === "meta" && !isFlorien(cliente) && subView === "dados" && (
        <CampanhasPanel
          clienteId={id}
          dateFilter={dateFilter}
          canal={canal}
          comparePrevious={hotelPilotEnabled}
          comparisonPreset={presetPeriodo}
        />
      )}

      {/* ── Google Ads: Campanhas (nível de campanha, aba Análise) ── */}
      {!analystOpen && canal === "google" && subView === "dados" && (
        <GoogleCampanhasPanel clienteId={id} filter={dateFilter} comparePrevious={hotelPilotEnabled} comparisonPreset={presetPeriodo} />
      )}

      {/* ── LinkedIn Ads: Campanhas ── */}
      {!analystOpen && canal === "linkedin" && (
        <LinkedInCampanhasPanel clienteId={id} filter={dateFilter} />
      )}
...
type MetaAdItem = {
  ...
  insights?: { data: Array<{ spend?: string; impressions?: string; clicks?: string; inline_link_clicks?: string; ctr?: string; cpc?: string; frequency?: string; actions?: Array<{ action_type: string; value: string }>; action_values?: Array<{ action_type: string; value: string }>; video_p100_watched_actions?: Array<{ action_type: string; value: string }> }> };
};
...
function MetaCriativosGrid({
  ...
  type CreativeSortKey = "name" | "spend" | "impressions" | "clicks" | "leads" | "cpl" | "purchases" | "cpa" | "attributedSalesValue" | "ctr" | "cr";
  const [tableSort, setTableSort] = React.useState<{ key: CreativeSortKey; direction: "asc" | "desc" }>({
    key: "spend",
    direction: "desc",
  });
  const sorted = React.useMemo(() => {
    ...
    return ads
      .map((ad) => {
        ...
        const actionValuesMap = new Map<string, number>();
        for (const row of insightRows) {
          for (const actionValue of row.action_values ?? []) {
            actionValuesMap.set(
              actionValue.action_type,
              (actionValuesMap.get(actionValue.action_type) ?? 0) + (parseFloat(actionValue.value) || 0)
            );
          }
        }
        ...
        const getActionValue = (type: string) => actionValuesMap.get(type) ?? 0;
        ...
        const comprasCount =
          ...
          getAction("purchase");
        const attributedSalesValue =
          getActionValue("purchase") ||
          getActionValue("omni_purchase") ||
          getActionValue("offsite_conversion.fb_pixel_purchase") ||
          getActionValue("website_purchase");
        const visitasCount =
  ...
  const hasAttributedSalesValue = sorted.some((item) => item.attributedSalesValue > 0);
  const hasSalesCampaigns = !isComprasPanel && sorted.some((item) =>
    item.purchases > 0 ||
    /\b(sale|sales|purchase|purchases|compra|compras|venda|vendas|checkout)\b/i.test(
      item.ad.adset?.campaign?.objective || item.ad.objective || item.ad.adset?.campaign?.name || ""
    )
  );
  ...
  const scoredItems = sorted
    .map((item) => {
      ...
      return { ...item, cpl, cpa, score, status, alerts: alerts.slice(0, 3), spShare };
    })
    .sort((a, b) => {
  ...
  const tableItems = (() => {
    const metricValue = (item: (typeof scoredItems)[number], key: CreativeSortKey): string | number | null => {
      if (key === "name") return item.displayName;
      if (key === "cpl") return item.leads > 0 ? item.cpl : null;
      if (key === "cpa") return item.purchases > 0 ? item.cpa : null;
      if (key === "attributedSalesValue") return item.attributedSalesValue > 0 ? item.attributedSalesValue : null;
      if (key === "cr") return item.leads > 0 ? item.cr : null;
      return item[key];
    };
    return [...scoredItems].sort((a, b) => {
      const aValue = metricValue(a, tableSort.key);
      const bValue = metricValue(b, tableSort.key);
      if (aValue === null && bValue === null) return a.displayName.localeCompare(b.displayName, "pt-BR");
  ...
  type PerformanceAggregate = {
    ...
    leads: number;
    purchases: number;
    attributedSalesValue: number;
    cpl: number | null;
    cpa: number | null;
    ctr: number;
    cr: number;
  };
  const aggregatePerformance = (items: TableItem[]): PerformanceAggregate => {
    const totals = items.reduce(
      (acc, item) => ({
        ...
        attributedSalesValue: acc.attributedSalesValue + item.attributedSalesValue,
      }),
      { spend: 0, impressions: 0, clicks: 0, leads: 0, purchases: 0, attributedSalesValue: 0 }
    );
    return {
      ...totals,
      ...
      cr: totals.clicks > 0 ? (totals.leads / totals.clicks) * 100 : 0,
    };
  };
  const hierarchy = (() => {
    type AdsetGroup = { id: string; name: string; items: TableItem[] };
    type CampaignGroup = { id: string; name: string; adsets: Map<string, AdsetGroup> };
    const campaigns = new Map<string, CampaignGroup>();
    for (const item of tableItems) {
      const campaignName = item.ad.adset?.campaign?.name?.trim() || "Campanha não identificada";
      const campaignId = item.ad.adset?.campaign?.id || `campaign:${campaignName}`;
      const adsetName = item.ad.adset?.name?.trim() || "Conjunto não identificado";
      const adsetId = item.ad.adset?.id || `${campaignId}:adset:${adsetName}`;
      let campaign = campaigns.get(campaignId);
      if (!campaign) {
        campaign = { id: campaignId, name: campaignName, adsets: new Map() };
        campaigns.set(campaignId, campaign);
      }
      let adset = campaign.adsets.get(adsetId);
      if (!adset) {
        adset = { id: adsetId, name: adsetName, items: [] };
        campaign.adsets.set(adsetId, adset);
      }
      adset.items.push(item);
    }
    const metricValue = (name: string, metrics: PerformanceAggregate, key: CreativeSortKey) => {
      if (key === "name") return name;
      if (key === "cpl") return metrics.cpl;
      if (key === "cpa") return metrics.cpa;
      return metrics[key];
    };
    const sortGroups = <T extends { name: string; metrics: PerformanceAggregate }>(groups: T[]) =>
      groups.sort((a, b) => {
        const av = metricValue(a.name, a.metrics, tableSort.key);
        const bv = metricValue(b.name, b.metrics, tableSort.key);
    ...
    return sortGroups(Array.from(campaigns.values()).map((campaign) => {
      const adsets = sortGroups(Array.from(campaign.adsets.values()).map((adset) => ({
        ...adset,
        metrics: aggregatePerformance(adset.items),
      })));
      const items = adsets.flatMap((adset) => adset.items);
      return { id: campaign.id, name: campaign.name, adsets, items, metrics: aggregatePerformance(items) };
    }));
  })();
  ...
  const renderMetricCells = (
    metrics: PerformanceAggregate,
    level: "campaign" | "adset",
    efficiencyReference?: number | null,
  ) => {
    const delta = metrics.cpl !== null && efficiencyReference
      ? ((metrics.cpl - efficiencyReference) / efficiencyReference) * 100
      : null;
    return (
      <>
  ...
  const changeTableSort = (key: CreativeSortKey) => {
    setTableSort((current) => ({
      key,
      direction: current.key === key && current.direction === "desc" ? "asc" : "desc",
    }));
  };
  const sortLabel: Record<CreativeSortKey, string> = {
    name: "Criativo",
    spend: "Investimento",
    impressions: "Impressões",
    clicks: "Cliques",
    leads: convLabels.chartKey,
    cpl: convLabels.metric,
    purchases: "Compras",
    cpa: "CPA",
    attributedSalesValue: "Venda atribuída",
    ctr: "CTR",
    cr: "Taxa de conversão",
  };
  const sortableHeader = (key: CreativeSortKey, label: string, align: "left" | "right" | "center" = "right", highlighted = false) => {
    const active = tableSort.key === key;
    const SortIcon = active ? (tableSort.direction === "asc" ? ArrowUp : ArrowDown) : ArrowUpDown;
    return (
      <th className={`${key === "name" ? "pl-6 pr-4" : "px-5"} py-2 ${align === "left" ? "text-left" : align === "center" ? "text-center" : "text-right"} ${highlighted ? "bg-[rgba(180,60,10,0.18)]" : ""}`}>
        <button
          type="button"
    ...
    <div className="space-y-5">
          ...
          <table className={`w-full text-left text-xs ${hasSalesCampaigns ? "min-w-[1120px]" : hasAttributedSalesValue ? "min-w-[900px]" : "min-w-[760px]"}`}>
            <thead>
              <tr className="border-b-2 border-[var(--border)]">
                ...
                {sortableHeader("spend", "Invest.")}
                {sortableHeader("impressions", "Impr.")}
                {sortableHeader("clicks", "Cliques")}
                {sortableHeader("leads", convLabels.chartKey)}
                {sortableHeader("cpl", convLabels.metric)}
                {hasSalesCampaigns && sortableHeader("purchases", "Compras")}
                {hasSalesCampaigns && sortableHeader("cpa", "CPA")}
                {hasAttributedSalesValue && sortableHeader("attributedSalesValue", "Venda atribuída")}
                {sortableHeader("ctr", "CTR")}
                {sortableHeader("cr", "Taxa Conv.", "center", true)}
              </tr>
            </thead>
            <tbody>
              {hierarchy.map((campaign) => {
                const campaignOpen = expandedCampaigns.has(campaign.id);
                const spendShare = totalSpend > 0 ? (campaign.metrics.spend / totalSpend) * 100 : 0;
                ...
                return (
                  <React.Fragment key={campaign.id}>
                    <tr className="border-b border-[var(--border)] bg-white/[0.045]">
                      ...
                      {renderMetricCells(campaign.metrics, "campaign")}
                    </tr>
                    {campaignOpen && campaign.adsets.map((adset) => {
                      const adsetOpen = expandedAdsets.has(adset.id);
                      const adsetSpendShare = campaign.metrics.spend > 0 ? (adset.metrics.spend / campaign.metrics.spend) * 100 : 0;
                      ...
                      return (
                        <React.Fragment key={adset.id}>
                          <tr className="border-b border-[var(--border)] bg-white/[0.018]">
                            ...
                            {renderMetricCells(adset.metrics, "adset", campaign.metrics.cpl)}
                          </tr>
                          {adsetOpen && adset.items.map((item) => {
                                  ...
                                  <div className="flex items-center gap-3">
                                    <div className="relative h-10 w-10 shrink-0 overflow-hidden rounded-lg bg-[var(--muted)]/30">
                                      {thumbUrl ? (
                                        ...
                                        <div className="flex h-full w-full items-center justify-center"><BarChart3 className="h-3.5 w-3.5 text-[var(--muted-foreground)]" /></div>
                                      )}
                                      {item.mediaType === "video" && (
                                        <div className="absolute inset-0 flex items-center justify-center bg-black/30"><Play className="h-3 w-3 text-white drop-shadow" /></div>
                                      )}
                                    </div>
      ...
      <div className="overflow-hidden rounded-[2rem] border border-[var(--border)] bg-[linear-gradient(180deg,rgba(20,21,26,0.98),rgba(12,12,16,1))] shadow-[0_24px_80px_rgba(0,0,0,0.38)]">
        ...
        <div className="space-y-3 p-5 sm:p-6">
          {(() => {
            const totalPurchases = hierarchy.reduce((sum, campaign) => sum + campaign.metrics.purchases, 0);
            const usePurchases = hasSalesCampaigns && totalPurchases > 0;
            const totalResults = usePurchases ? totalPurchases : totalLeads;
            const resultLabel = usePurchases ? "compras" : convLabels.chartKey.toLowerCase();
            return [...hierarchy]
              .sort((a, b) => b.metrics.spend - a.metrics.spend)
              .map((campaign) => {
                const results = usePurchases ? campaign.metrics.purchases : campaign.metrics.leads;
                const spendShare = totalSpend > 0 ? (campaign.metrics.spend / totalSpend) * 100 : 0;
                const resultShare = totalResults > 0 ? (results / totalResults) * 100 : 0;
