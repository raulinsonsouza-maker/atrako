...
function ConnectWizard() {
  ...
  useEffect(() => {
    function onMessage(event: MessageEvent) {
      ...
      if (data.ok) {
        setOauthError(null);
        router.replace("/social/connect?step=5");
        setStep(2);
        void (async () => {
          const acc = await loadAccount();
          setStep(3);
  ...
  return (
    <div className="p-6 md:p-10">
      <h1 className="type-tagline">Conectar Instagram</h1>
      ...
      <div className="mt-8 flex flex-wrap gap-2">
        {STEPS.map((label, i) => {
          ...
          return (
            <div
              key={label}
              className={`flex items-center gap-2 rounded-full px-3 py-1.5 type-fine-print ${
                active
        ...
        {step === 4 && (
          <div className="text-center">
            ...
            <div className="mt-8 flex flex-col items-center gap-3 sm:flex-row sm:justify-center">
              ...
              <button
                type="button"
                onClick={() => router.push("/social/flows/new")}
