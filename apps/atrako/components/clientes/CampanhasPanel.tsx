...
interface Campanha {
  ...
  roas: number | null;
  ctr: number | null;
  comparison?: { state: "new" | "continuing" | "stopped"; investimento: number | null; faturamento: number | null; purchases: number | null };
}
...
export function CampanhasPanel({ clienteId, dateFilter, canal = "geral", mqlByCampaignName, mqlByAdsetId, mqlByAdId, comparePrevious = false, comparisonPreset }: Props) {
  ...
  const params = buildParams(dateFilter, {
    ...
    ...(comparePrevious && nivel === "campanhas" ? { compare: "previous" } : {}),
    ...(comparePrevious && nivel === "campanhas" && comparisonPreset ? { comparisonPreset } : {}),
  });

  const { data, isLoading, isError, error } = useQuery({
    queryKey: ["campanhas", clienteId, nivel, selectedCampanha, selectedConjunto, dateFilter, comparePrevious ? "pilot-comparison" : "standard", comparisonPreset ?? "none"],
    queryFn: async () => {
      let res = await fetch(`/api/clientes/${clienteId}/campanhas?${params}`);
      if (res.status === 401 && comparePrevious && nivel === "campanhas") {
        const standardParams = new URLSearchParams(params);
        standardParams.delete("compare");
        standardParams.delete("comparisonPreset");
        res = await fetch(`/api/clientes/${clienteId}/campanhas?${standardParams}`);
      }
