...
interface Campanha {
  ...
  statusOrder?: number;
  comparison?: { state: "new" | "continuing" | "stopped"; investimento: number | null; conversoes: number | null; conversaoValor: number | null };
}
...
interface Props { clienteId: string; filter: DateFilter; comparePrevious?: boolean; comparisonPreset?: string; }
...
export function GoogleCampanhasPanel({ clienteId, filter, comparePrevious = false, comparisonPreset }: Props) {
  ...
  const { data, isLoading, error } = useQuery<{ campanhas: Campanha[] }>({
    ...
    queryFn: async () => {
      ...
      if (r.status === 401 && comparePrevious) {
        const standardParams = new URLSearchParams(campParams);
        standardParams.delete("compare");
        standardParams.delete("comparisonPreset");
        r = await fetch(`/api/clientes/${clienteId}/campanhas-google?${standardParams}`);
      }
