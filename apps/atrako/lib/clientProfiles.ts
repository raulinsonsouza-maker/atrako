...
/** Exact identity for the development-only Conta Hotel pilot (not the hotel template). */
export function isContaHotelPilot(client?: ClientIdentity | null) {
  if (!client) return false;
  const slug = normalizeText(client.slug);
  const nome = normalizeText(client.nome);
  return slug === "conta-hotel" || nome === "conta hotel";
}
...
export function isImobClient(client?: ClientIdentity | null) {
  ...
  return (
    isMiranteIncorporadora(client) ||
    isMiguelImoveis(client) ||
    isSouIcarai(client)
  );
}

export function isSocialMediaOnly(client?: ClientIdentity | null) {
  if (!client) return false;
  return client.perfilPanel === "social-media";
}
