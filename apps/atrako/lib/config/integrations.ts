...
export async function getIntegrationsConfig(): Promise<IntegrationsConfig> {
  ...
  return {
    ...
    telegramBotToken: map.get(KEYS.telegramBotToken) ?? null,
    telegramChannelId: map.get(KEYS.telegramChannelId) ?? null,
  };
}
...
export async function updateIntegrationsConfig(data: {
  ...
  if (data.telegramBotToken !== undefined) {
    ops.push(
      prisma.systemConfig.upsert({
        where: { key: KEYS.telegramBotToken },
        create: { key: KEYS.telegramBotToken, value: data.telegramBotToken.trim() },
        update: { value: data.telegramBotToken.trim() },
      })
    );
  }
  ...
  if (data.telegramChannelId !== undefined) {
    ops.push(
      prisma.systemConfig.upsert({
        where: { key: KEYS.telegramChannelId },
        create: { key: KEYS.telegramChannelId, value: data.telegramChannelId.trim() },
        update: { value: data.telegramChannelId.trim() },
      })
    );
  }
