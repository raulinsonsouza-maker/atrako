...
export interface CrmFilters {
  ...
  /** When true, only Meta + Google leads are shown (paid media only). */
...
export async function getCrmFilters(clienteId: string): Promise<CrmFilters> {
  ...
  if (!config || !config.ativo) {
    return { tagFilter: [], conversaoOriginalFilter: [], conversaoUltimoFilter: [], midiaFilter: [], origemOriginalFilter: [], origemUltimoFilter: [], paidMediaOnly: false };
  }
  ...
  return {
    ...
    // paidMediaOnly: explicit flag OR derived from allowedSources (e.g. ["meta ads","google ads"])
    paidMediaOnly: creds.paidMediaOnly === true ||
      (toStringArray(creds.allowedSources).length > 0 &&
       toStringArray(creds.allowedSources).every((s) =>
         /meta|google|facebook|instagram|youtube/i.test(s)
       )),
  };
}
