...
const GRAPH_PREVIEW_BASE = `https://graph.facebook.com/${META_GRAPH_VERSION}`;
...
export async function fetchAccountBalance(accountId: string, token: string): Promise<MetaAccountBalance> {
  ...
  const isPrepaid = Number(fsd?.type) === 20;
  if (isPrepaid && fsd?.display_string) {
  ...
  // 2. fsdParsed === 0 → prepaid account explicitly reporting zero → show 0
  // 3. accountBalance > 0 with no fsd type 20 → credit card/other, use it
  // 4. Unparseable prepaid response or everything null → return null, preserving cache
  const balance: number | null = isPrepaid
    ? fsdParsed
    : accountBalance > 0
    ? accountBalance
    : null;
