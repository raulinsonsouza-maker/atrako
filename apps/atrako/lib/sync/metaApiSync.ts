import { fetchCampaignInsightsAggregatedByDay, fetchCampaignInsightsPerCampaign, fetchAdsWithCreatives, fetchAccountBalance } from "@/lib/meta/metaClient";
...
export async function syncMetaCliente(
  ...
  if (await isSyntheticDemoCliente(clienteId)) {
    return { daysProcessed: 0, creativesProcessed: 0 };
  }
  ...
  try {
    ...
    // Atualiza saldo da conta — fire-and-forget, nunca bloqueia o sync principal
    if (conta?.id) {
      try {
        const balanceData = await fetchAccountBalance(accountId, token);
        // Persiste mesmo quando balance é null (conta pré-paga sem parse ainda),
        // garantindo que saldoAtualizadoAt reflita a última tentativa e não mostre valor stale.
        await prisma.conta.update({
          where: { id: conta.id },
          data: { saldoAtual: balanceData.balance, saldoAtualizadoAt: new Date() },
        });
      } catch {
        // Silencioso — falha de saldo não afeta os dados de campanha
      }
    }

    await markMetaConnectionHealth(clienteId, "ready", null).catch(() => null);
...
/**
 * Sync META for all active clients.
 * Each client uses its own WorkspaceConnection / Conta credentials (isolated failures).
 */
export async function syncMetaTodosClientes(options?: { dateFrom?: string; dateTo?: string }): Promise<MetaSyncAllResult[]> {
  ...
  for (const cliente of clientes) {
    try {
      const resolved = await resolveMetaCredentials(cliente.id);
      if (!resolved?.token) {
        results.push({
      ...
      if (!resolved.accountId) {
        results.push({
          ...
          daysProcessed: 0,
          creativesProcessed: 0,
          error: "Sem conta de anúncio Meta selecionada",
        });
        continue;
      }
