...
## Main Sections
- `/clientes` — Central pública de Clientes: dashboards agregados de performance por cliente
- `/gestao` — Gestão operacional interna: login obrigatório por conter campos editáveis, orçamento, pagamento e saldo
...
- `/planejamento` — Central pública de Planejamento: planos táticos/estratégicos para apresentação a clientes
...
- `SESSION_SECRET` - Assinatura e proteção das sessões internas locais

## Política de acesso
- Leituras agregadas dos dashboards em `/clientes`, Meta, Google, Analytics, Social e Planejamento são públicas.
- CRM com dados pessoais, lead scoring individual, financeiro detalhado, pautas e chamadas ao vivo aos provedores exigem sessão interna ou portal válido do mesmo cliente.
- `/gestao`, `/admin`, InPilot, configurações, sincronizações, importações, OAuth e qualquer mutação exigem sessão interna persistente; operações administrativas exigem papel `ADMIN`.
- Endpoints públicos devem optar explicitamente por `public-read`; a política padrão de cliente é negar acesso anônimo.
...
- API `POST /api/admin/clientes/[id]/regenerate-token` — gera novo UUID e invalida o link anterior (requer sessão local de administrador)
...
- API: todas as rotas `/api/admin/segmentos` exigem sessão local de administrador
...
- **Janela de lookback incremental Google Ads: 30 dias** (não 3). PMax usa atribuição data-driven com janela de 30 dias — conversões tardias são retroativamente atribuídas ao dia do clique. 3 dias subestimava compras/faturamento em ~16%. A cada sync incremental, o dateFrom = lastFatoDate − 30d, re-fazendo upsert dos últimos 30 dias.
...
- Limitações conhecidas: o sync por-cliente só atualiza o cliente aberto. A cobertura de "todos + alertas" é feita pelo disparo global abaixo. O endpoint de sync por cliente exige sessão interna.
